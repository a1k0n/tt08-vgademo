...
More detailed write-up available [on my blog](https://www.a1k0n.net/2025/12/19/tiny-tapeout-demo.html).
...
The plane is rendered by doing a bit-by-bit [non-restoring
division](https://en.wikipedia.org/wiki/Division_algorithm#Non-restoring_division)
of the screen $y$ coordinate during the horizontal blanking interval to find a
fixed point reciprocal, which is then used as a per-horizontal-pixel increment
for the plane $u$ coordinate.  As a drastic simplification, the plane $v$
coordinate is *also* the $x$ increment value (when you do the math, it turns out
...
The "TT08" logo uses the outline and palette of an old demo font, but the actual
coloring is procedural as it would take too much combinational logic to
reproduce exactly.
...
noise. Only 8 unique notes are used in the scale (C major, plus F# for D major
acting as a V-of-V chord -- the progression is iv-I-II-IV-V), and only 2 octaves
are supported on each channel to minimize the number of shifts/muxes needed.
